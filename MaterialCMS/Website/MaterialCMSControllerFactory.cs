...
{
    public class MaterialCMSControllerFactory : DefaultControllerFactory
    {
        ...
        static MaterialCMSControllerFactory()
        {
            AppUiControllers =
                MaterialCMSApp.AppTypes.Where(pair => typeof(MaterialCMSUIController).IsAssignableFrom(pair.Key))
                    .GroupBy(pair => pair.Value)
                    .ToDictionary(grouping => grouping.Key,
                        grouping => grouping.Select(pair => pair.Key).ToHashSet());
            AppAdminControllers =
                MaterialCMSApp.AppTypes.Where(pair => typeof(MaterialCMSAdminController).IsAssignableFrom(pair.Key))
                    .GroupBy(pair => pair.Value)
                    .ToDictionary(grouping => grouping.Key,
                        grouping => grouping.Select(pair => pair.Key).ToHashSet());
            UiControllers = new HashSet<Type>(
                TypeHelper.GetAllConcreteTypesAssignableFrom<MaterialCMSUIController>()
                    .Where(type => !AppUiControllers.SelectMany(pair => pair.Value).Contains(type)));
            AdminControllers = TypeHelper.GetAllConcreteTypesAssignableFrom<MaterialCMSAdminController>()
