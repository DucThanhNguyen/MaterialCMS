...
namespace MaterialCMS.Helpers
{
    public static class TypeHelper
    {
        ...
        {
            return _alltypes ??
                   (_alltypes = GetAllMaterialCMSAssemblies().SelectMany(GetLoadableTypes).Distinct().ToHashSet());
        }

        public static HashSet<Assembly> GetAllMaterialCMSAssemblies()
        {
            return
                _mrCMSAssemblies =
                _mrCMSAssemblies ??
                AppDomain.CurrentDomain.GetAssemblies()
                         .Where(assembly => assembly.GetCustomAttributes<MaterialCMSAssemblyAttribute>().Any()).ToHashSet();
        }
